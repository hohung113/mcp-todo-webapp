...
import { getTodos, createTodo, deleteTodo, completeTodo, sendToAgent } from './api/todoApi'
...
export default function App() {
  ...
  async function loadTodos() {
    try {
      ...
      setError('Cannot connect to API. Make sure ASP.NET is running on port 7012.')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className={styles.layout}>
      ...
      <main className={styles.main}>
        <form className={styles.form} onSubmit={handleAdd}>
          <input
            ...
            autoFocus
          />
          <button className={styles.addBtn} type="submit" disabled={adding || !input.trim()}>
            {adding ? '...' : '+ ADD'}
          </button>
        </form>
        ...
        <div className={styles.aiSection}>
          ...
          <form className={styles.aiForm} onSubmit={handleAiSubmit}>
            <input
              ...
              disabled={aiLoading}
            />
            <button className={styles.aiBtn} type="submit" disabled={aiLoading || !aiInput.trim()}>
              {aiLoading ? <span className={styles.aiSpinner}>⟳</span> : 'SEND'}
            </button>
          </form>
          {aiResult && (
            <div className={styles.aiResponse}>
              {aiResult.action === 'created' && <span>✅ AI đã tạo: <strong>{aiResult.title}</strong></span>}
              {aiResult.action === 'none' && <span>💬 {aiResult.message}</span>}
              {aiResult.action === 'deleted' && <span>🗑️ AI đã xóa todo</span>}
              {aiResult.action === 'completed' && <span>✔️ AI đã hoàn thành todo</span>}
            </div>
          )}
        </div>
        ...
        {loading ? (
          ...
          <div className={styles.list}>
            ...
            {completed.length > 0 && pending.length > 0 && (
              <div className={styles.divider}><span>COMPLETED</span></div>
            )}
            {completed.map(todo => (
              <TodoItem key={todo.id} todo={todo} onComplete={handleComplete} onDelete={handleDelete} />
            ))}
      ...
      <footer className={styles.footer}>
        <span>TODOAI — powered by ASP.NET + React</span>
      </footer>
    </div>
  )
}
